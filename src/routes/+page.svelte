<script>
  ...
  const preloadImageUrls = products.map(product => {
    ...
    for (let i=0;i<flipbookLength;i++) { 
      ...
      const staticSrcDesktop = `/products/${product.id}/flipbook/desktop/${product.filePrefix}${i< 10 ? "0"+i : i}.png`
      const staticSrcMobile = `/products/${product.id}/flipbook/mobile/${product.filePrefix}${i< 10 ? "0"+i : i}.png`
    ...
    console.log(vercelBgDesktop)
    console.log(vercelBgMobile)
    return productImages;
  }).flat()

  const preloadVideoUrls = products.map(product => {
    return [
      `https://pub-4f07e87cd034401c89285d81277106a8.r2.dev/${product.id}-product-bg-compressed.mp4`
    ];
  }).flat()
...
<Scroller top={animThreshold} bottom={animThreshold} threshold={animThreshold} bind:index bind:offset bind:progress>
  ...
  <div class="foreground-slot" slot="foreground" bind:this={scrollContainer}>
    ...
    {#each products as product, i}
      ...
      <section data-scroll-onscreen data-scroll-node data-lifestyle-bg={i+1} class="sticky top-0 z-20 overflow-hidden" style="visibility:{index > (i*sectionsPerProduct-2) ? "visible":"hidden"};">
